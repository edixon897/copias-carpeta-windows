...
credentials_global = {'username': '', 'password': '', 'domain': ''}
...
# Función para verificar si se tiene acceso a una carpeta
def verificar_acceso(carpeta):
    try:
        os.listdir(carpeta)
        return True
...
def copiar_carpeta_handler(ip_origen, ip_destino):
    ...
    def execute_copy():
        ...
        result, handle = logon_user(credentials['username'], credentials['password'], credentials['domain'])
        if not result:
            messagebox.showerror("Error", "No se pudo autenticar con las credenciales proporcionadas al inicio.")
            return
        try:
...
# Solicitar credenciales de red al inicio
credentials_global = request_credentials("red")
